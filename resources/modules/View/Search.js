...
class Search {
  ...
  initHosts() {
    this.searchField = this.search.querySelector(`.search__item`);
    this.searchRunner = this.search.querySelector(`.search__btn`);
  }

  initlisteners() {
    this.searchRunner.addEventListener(`click`, () => {
      this.drawFoundPosts(this.startSearch());
    });
  ...
  drawFoundPosts(posts) {
    this.target.innerHTML = posts.length
      ? postTemplate(posts)
      : `<div class="post-row_nofound">The search returned no results, please try another value in the search bar</div>`;
  }
